const Accountant = require("../model/AccountHeadModel");
...
exports.accountantLogin = async (req, res) => {
  ...
  try {
    ...
    if (!name || !password) {
      return res
        .status(400)
        .json({ message: "Accountant Head Name and Password are required" });
    }

    // Check if the Accountant exists
    const existingAccountant = await Accountant.findOne({ name });
    if (!existingAccountant) {
      return res.status(400).json({ message: "Accountant Head does not exist" });
    }
    ...
    console.log("Accountant Head login successful:", existingAccountant);
  } catch (error) {
    // Handle validation and other errors
    console.error("Accountant Head  login failed:", error);
    res
      .status(500)
      .json({ message: "Accountant Head  login failed", error: error.message });
  }
};
